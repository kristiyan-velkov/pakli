...
export async function registerUser(user: User): Promise<Partial<User>> {
  ...
  const { data: userProfileData, error: userProfileError,  } = await supabaseAdminClient.from("user_profile").insert({
    id: authUser.user.id,
  ...
  if (!userProfileData) {
    throw new Error("User profile creation failed: userProfileData is null.");
  }
